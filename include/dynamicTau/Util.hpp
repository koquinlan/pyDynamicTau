...
{
  torch::Tensor result = torch::empty({(long)v.size(), (int64_t)Feature().size()});
...
{
  bool* a = new bool[v.size()];
  std::copy(v.begin(), v.end(), a);
  torch::Tensor result = torch::from_blob(a, {(long)v.size(), 1}, torch::kBool).clone();
