...
class ActorBeamsplitter : Actor<Feature> {
  ...
  void Construct(int seed) {
      ...
      dqn = Network(Feature().size(), numHidden, EnvironmentBeamsplitter::numberOfPossibleActions);
      dqn->initializeParameters();
      tdOptimizer = new torch::optim::Adam(dqn->parameters(),
                                           torch::optim::AdamOptions(learningRate));

      target = Network(Feature().size(), numHidden, EnvironmentBeamsplitter::numberOfPossibleActions);
...
public:
  ...
  explicit ActorBeamsplitter(int seed)
  {
      Construct(seed);
  }
...
int ActorBeamsplitter::proposeAction(const Feature& state, const double epsilon)
{
  ...
  if (chance >= epsilon) {
    ...
    return randomPolicy(EnvironmentBeamsplitter::numberOfPossibleActions, generator);
}

torch::Tensor ActorBeamsplitter::tdLoss(const Sample<Feature>& sample)
{
...
double ActorBeamsplitter::learn()
{
